<template>
<v-card :to="`${post.url}`" :target="`${post.ext}`" rel="noreferrer" hover class="text-left card-shadow">
  <v-img :src="`${post.img}`" height="340" />
  <h1 class="title ma-4" color="primary--text">
    {{ post.title }}
  </h1>
  ...
  <v-card-text color="primary--text">
    {{ post.text }}
  </v-card-text>
