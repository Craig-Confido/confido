  ...
  <v-container fluid class="pa-0 ma-0" v-if="home.metadata && startups.metadata">
    <section class="mx-4 mx-lg-16">
      <v-row class="text-left mt-4">
        <v-img :src="require('../static/images/startupsVisual.png')" class="d-flex d-md-none mx-md-16" alt="image of people" />
        <v-col cols="12" sm="7" class="mt-md-16">
          <h1 class="text-h3 text-md-h2 font-weight-bold" aria-label="Intro text">
            {{ startups.title }}
          </h1>
          <h2 class="text-h5 my-8">Whether you're making a critical hire, building a core team or scaling up squads, we're here for you.</h2>
          <v-row class="mt-12 mx-0">
            <v-btn class="mt-0 mr-4 btn--outlined font-weight-bold" outlined color="accent" rounded width="150" height="50" @click="$vuetify.goTo('#search')"> Search </v-btn>
            <v-btn class="mt-0 mr-4 btn--outlined font-weight-bold" outlined color="success" rounded width="150" height="50" @click="$vuetify.goTo('#onsite')"> Onsite </v-btn>
          </v-row>
        </v-col>
        <v-spacer />
        <v-col cols="12" sm="5">
          <v-img :src="require('../static/images/startupsVisual.png')" class="d-none d-md-flex mx-md-16" alt="image of people" />
        </v-col>
      </v-row>
    </section>
    ...
    <v-row id="search" class="text-left mt-8 mt-md-0 mx-0 mx-md-6 mx-lg-16">
      <v-col cols="12">
        <h1>Search.</h1>
      </v-col>
      <v-col cols="12" sm="8">
        <h2>Working exclusively with Seed and Series A startups to efficiently deliver a shortlist of three candidates within four weeks that you will make a hire from.</h2>
      </v-col>
    ...
    <v-row class="mx-0 mx-md-6 mx-lg-16">
      <v-col cols="12">
        <v-card class="pa-8 my-2 white--text text-left card-shadow">
          ...
          <v-card-text class="text-h6 primary--text"> We know it can be hard to attract top talent when you have no brand presence in the market. Search allows you to make critical hires efficiently and effortlessly, at pace. We work with you to craft a compelling message, then headhunt the passive talent market to find you the best. </v-card-text>
          <v-row class="primary--text mx-auto text-center mt-4">
            <v-col cols="6" sm="3">
              <v-icon class="secondary--text text-h2"> mdi-account-group-outline </v-icon>
              <h1 class="text-h5 text-md-h4">{{ startups.metadata.why_search.candidate }} candidate</h1>
              <h3 class="text-md-h6">Limit per role</h3>
            </v-col>
            <v-col cols="6" sm="3">
              <v-icon class="secondary--text text-h2"> mdi-clock-check-outline </v-icon>
              <h1 class="text-h5 text-md-h4">{{ startups.metadata.why_search.time_to_hire }} weeks</h1>
              <h3 class="text-md-h6">Average time to hire</h3>
            </v-col>
            <v-col cols="6" sm="3">
              <v-icon class="secondary--text text-h2"> mdi-check-circle-outline </v-icon>
              <h1 class="text-h5 text-md-h4">{{ startups.metadata.why_search.hires_made }} of hires</h1>
    ...
    <v-row class="mx-0 mx-md-6 mx-lg-16">
      <v-col cols="12">
        <ColorWideCard v-for="(card, index_1) in purpleCards" :key="card.title + index_1" :card="card" />
      </v-col>
    </v-row>

    <v-row>
      <v-col cols="12" md="12" class="mt-8 text-center">
        <p class="text-h4 font-weight-bold">Case studies.</p>
        <v-card-text class="primary--text text-h5"> Learn more from the teams we've built </v-card-text>
            ...
            <v-dialog width="1000px" class="overflow-hidden" v-model="benefex">
              <template v-slot:activator="{ on, attrs }">
                <div class="circle pointer my-8">
                  <v-img :src="require('../static/images/benefex.png')" width="150" contain eager class="circle pointer" v-bind="attrs" v-on="on" />
                </div>
              </template>
              ...
              <CaseStudyLargeCard v-for="(card, index_2) in caseStudyCards.slice(0, 1)" :key="card.title + index_2" :card="card" />
            </v-dialog>
          </v-col>
          <v-col cols="12" md="4" lg="2">
            <v-dialog width="1000px" class="overflow-hidden" v-model="thirdSpace">
              <template v-slot:activator="{ on, attrs }">
                <div class="circle pointer my-8">
                  <v-img :src="require('../static/images/thirdSpace.png')" width="150" contain eager class="circle pointer" v-bind="attrs" v-on="on" />
                </div>
              </template>
              <div style="position: fixed; width: 1000px; z-index: 2">
                <v-col cols="1" class="mr-auto">
                  <v-icon large color="white" @click="thirdSpace = false"> mdi-close-circle-outline </v-icon>
                </v-col>
              </div>
              <CaseStudyLargeCard v-for="(card, index_3) in caseStudyCards.slice(1, 2)" :key="card.title + index_3" :card="card" />
            </v-dialog>
          </v-col>
          <v-col cols="12" md="4" lg="2">
            <v-dialog width="1000px" class="overflow-hidden" v-model="allplants">
              <template v-slot:activator="{ on, attrs }">
                <div class="circle pointer my-8">
                  <v-img :src="require('../static/images/allplants.png')" width="150" contain eager class="circle pointer" v-bind="attrs" v-on="on" />
                </div>
              </template>
              <div style="position: fixed; width: 1000px; z-index: 2">
                <v-col cols="1" class="mr-auto">
                  <v-icon large color="white" @click="allplants = false"> mdi-close-circle-outline </v-icon>
                </v-col>
              </div>
              <CaseStudyLargeCard v-for="(card, index_4) in caseStudyCards.slice(2, 3)" :key="card.title + index_4" :card="card" />
            </v-dialog>
          </v-col>
          <v-spacer />
        </v-row>
    ...
    <v-row id="onsite" class="text-left mx-0 mx-md-6 mx-lg-16">
      ...
      <v-col cols="12" sm="7">
        <h2>Partnering with Series B and C startups to enable sustainable long-term scaling of product and tech teams.</h2>
      </v-col>
    ...
    <v-row class="mx-0 mx-md-6 mx-lg-16">
      <v-col cols="12">
        <ColorWideCard v-for="(card, index_5) in greenCards" :key="card.title + index_5" :card="card" />
      </v-col>
    </v-row>
        ...
        <v-row class="my-4 mx-md-0 text-center">
          ...
          <v-col cols="12" md="4" lg="2">
            <v-dialog width="1000px" class="overflow-hidden" v-model="curve">
              <template v-slot:activator="{ on, attrs }">
                <div class="circle pointer my-8">
                  <v-img :src="require('../static/images/curve.png')" width="150" contain eager class="circle pointer" v-bind="attrs" v-on="on" />
                </div>
              </template>
              <div style="position: fixed; width: 1000px; z-index: 2">
                <v-col cols="1" class="mr-auto">
              ...
              <CaseStudyLargeCard v-for="(card, index_6) in caseStudyCards.slice(3, 4)" :key="card.title + index_6" :card="card" />
            </v-dialog>
          </v-col>
          <v-col cols="12" md="4" lg="2">
            <v-dialog width="1000px" class="overflow-hidden" v-model="unmind">
              <template v-slot:activator="{ on, attrs }">
                <div class="circle pointer my-8">
                  <v-img :src="require('../static/images/unmind.png')" class="circle" width="150" contain eager v-bind="attrs" v-on="on" />
                </div>
              </template>
              <div style="position: fixed; width: 1000px; z-index: 2">
                <v-col cols="1" class="mr-auto">
              ...
              <CaseStudyLargeCard v-for="(card, index_7) in caseStudyCards.slice(4, 5)" :key="card.title + index_7" :card="card" />
            </v-dialog>
          </v-col>
          <v-col cols="12" md="4" lg="2">
            <v-dialog width="1000px" class="overflow-hidden" v-model="feedr">
              <template v-slot:activator="{ on, attrs }">
                <div class="circle pointer my-8">
                  <v-img :src="require('../static/images/feedr.webp')" width="150" contain eager class="circle" v-bind="attrs" v-on="on" />
                </div>
              </template>
              <div style="position: fixed; width: 1000px; z-index: 2">
                <v-col cols="1" class="mr-auto">
              ...
              <CaseStudyLargeCard v-for="(card, index_8) in caseStudyCards.slice(5, 6)" :key="card.title + index_8" :card="card" />
            </v-dialog>
          </v-col>
          <v-spacer />
        </v-row>
...
export default {
      ...
      caseStudyCards: [
        {
          ...
          resultText: 'We adjusted the search based on the market intel and made all 3 hires within 6 weeks. Since then, we’ve continued to work exclusively on all product and engineering hires.',
        ...
        },
        {
          ...
          challenge2: 'Being an early stage startup they weren’t sure on how product and tech talent wanted to be engaged and what mattered to them.',
        ...
        },
        {
          ...
          title: 'allplants was created to make eating more plants exciting and easy, delivering delightful, healthier food to busy homes everywhere.',
          url: 'https://www.allplants.com/',
          brief: 'allplants needed to split their product and engineering team into a more scalable structure. They needed to move to a squad model with self-contained product and engineering teams, aligned to areas of the business with their own product roadmap.',
          challenge1Title: 'Product hiring',
          challenge1: 'allplants had been trying to hire both Product Manager roles directly for over two months with little success.',
          challenge2Title: 'Head of Data hiring',
          challenge2: 'This was a brand-new role for BOTH the business and the main stakeholder (Finance Director).',
          solution1Icon: 'mdi-apache-kafka',
          solution1Title: 'Product hiring',
          solution1Text: 'We agreed these roles were at the wrong salary and seniority bandings; the Head of Product then got them approved at Lead level and we were able to make the hires in four weeks (on average).',
          solution2Icon: 'mdi-finance',
          solution2Title: 'Head of Data hiring',
          solution2Text: 'We advised the stakeholder and Head of Talent that there were two types of Head of Data we could hire: someone stepping up into the role or a more experienced candidate. The stakeholder agreed that the first type was best suited and by week 8 the hire was made.',
          resultText: 'We worked with allplants’ four stakeholders to successfully made all 10 hires with a 4.8 week average time to hire. Our focus throughout this process was balancing consistency, quality and responsiveness to ensure an excellent candidate and stakeholder experience.',
        ...
        },
        {
          ...
          challenge2: 'Their existing data was not fit for purpose, which lead to too many “gut feel” decisions.',
  ...
  methods: {
    ...
    async getStartupsData() {
      ...
      const data = await bucket.objects
        .findOne({
          id: '64ce853f07970f7296000a91',
        })
        .props('title,metadata');
      this.startups = await data.object;
      this.loading = false;
    },
  },
};
